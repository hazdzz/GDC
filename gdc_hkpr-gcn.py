...
def get_parameters():
    parser = argparse.ArgumentParser(description='GDC(HKPR)-GCN')
    ...
    parser.add_argument('--model', type=str, default='gdc_hkpr-gcn', help='graph neural network model')
    ...
    parser.add_argument('--t', type=float, default=3, help='diffusion time')
    ...
    if mode == 'tuning':
        ...
        t, K, lr, weight_decay, droprate = [*param.values()]
    else:
        if args.t <= 0:
    ...
    model_save_path = model_name + '_' + gso_type + '_' + str(t) + '_t_' + str(K) + '_layer' + '.pth'
    model_save_path = os.path.join(model_save_dir, model_save_path)

    return device, dataset, model_name, gso_type, t, lr, weight_decay, droprate, n_hid, enable_bias, K, epochs, opt, early_stopping_patience, model_save_path

def process_data(device, dataset, gso_type, t):
    ...
    if device == torch.device('cpu'):
        filter = utility.calc_hkpr_filter(gso, t, device)
        filter = filter.astype(dtype=np.float32)
        filter = torch.from_numpy(filter).to(device)
    else:
        gso = utility.cnv_sparse_mat_to_coo_tensor(gso, device)
        filter = utility.calc_hkpr_filter(gso, t, device)
        filter = filter.type(torch.cuda.FloatTensor)
...
def prepare_model(n_feat, n_hid, n_class, enable_bias, K, droprate, \
    ...
    model = models.GDCGCN(n_feat, n_hid, n_class, enable_bias, K, droprate).to(device)
...
if __name__ == "__main__":
    device, dataset, model_name, gso_type, t, lr, weight_decay, droprate, n_hid, enable_bias, K, epochs, opt, early_stopping_patience, model_save_path = get_parameters()
    feature, filter, label, idx_train, idx_val, idx_test, n_feat, n_class = process_data(device, dataset, gso_type, t)
