...
def get_parameters():
    parser = argparse.ArgumentParser(description='GDC(PPR)-GCN')
    ...
    parser.add_argument('--model', type=str, default='gdc_ppr-gcn', help='graph neural network model')
    ...
    if mode == 'tuning':
        ...
        if args.alpha >= 1 or args.alpha < 0:
            raise ValueError(f'ERROR: The restart probability alpha for PPR should be in (0, 1)!')
        else:
            alpha = args.alpha
        if args.K < 0:
    ...
    model_save_path = model_name + '_' + gso_type + '_' + str(alpha) + '_alpha_' + str(K) + '_layer' + '.pth'
    model_save_path = os.path.join(model_save_dir, model_save_path)

    return device, dataset, model_name, gso_type, alpha, lr, weight_decay, droprate, n_hid, enable_bias, K, epochs, opt, early_stopping_patience, model_save_path
...
if __name__ == "__main__":
    device, dataset, model_name, gso_type, alpha, lr, weight_decay, droprate, n_hid, enable_bias, K, epochs, opt, early_stopping_patience, model_save_path = get_parameters()
